class AritclesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @aritcle.save
        format.html { redirect_to @aritcle, notice: "Aritcle was successfully created." }
  ...
  def update
    respond_to do |format|
      if @aritcle.update(aritcle_params)
        format.html { redirect_to @aritcle, notice: "Aritcle was successfully updated." }
  ...
  private
    ...
    def aritcle_params
      params.require(:aritcle).permit(:title, :post, :user_id)
    end
end
